const load = async () => {
  const us = await d3.json('https://cdn.jsdelivr.net/npm/us-atlas@2/us/10m.json');
  const states = new Map(us.objects.states.geometries.map(d => [d.id, d.properties]));
  const data = Object.assign(new Map(await d3.csv('data.csv', ({id, price}) => [id, +price])));
  draw(us, states, data);
}
...
function scale (factor, width, height) {
  return d3.geoTransform({
    point: function(x, y) {
      this.stream.point((x-width/5)*factor + width/5, (y-height/20)*factor + height/20);
    }
  });
}
...
const draw = (us, states, data) => {
  ...
  const map = svg.append('g')
    ...
    .join('path')
    .attr("fill", d => data.get(d.id) == null ? '#d89cf6' : color(data.get(d.id)))
    .attr('stroke', 'white')
