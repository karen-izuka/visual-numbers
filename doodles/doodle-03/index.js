...
const draw = data => {
  ...
  const node = svg
    ...
    .on('mouseover', function (e, d) {
      div.transition().duration(20).style('opacity', 0.8);
      div
        .html(
          `<div>${d.name.replace(/0 |1 |2 /g, '')}</div><div>${format(
            d.value
          )} villagers</div>`
        )
  ...
  link
    ...
    .on('mouseover', function (e, d) {
      ...
      div
        .html(
          `<div>${d.source.name.replace(
            /0 |1 |2 /g,
            ''
          )} → ${d.target.name.replace(/0 |1 |2 /g, '')}</div><div>${format(
            d.value
          )} villagers</div>`
        )
        .style('left', `${e.pageX}px`)
