...
const generateTable = (table, data) => {
  ...
  for (let el of data) {
    ...
    cell.innerHTML =
      el.tag == "level-01" ? "<i class='fa fa-caret-right'></i>" : "";
  ...
  $(document).ready(function () {
    $(".level-01").click(function () {
      if ($(this).next().css("visibility") == "collapse") {
        $(this).nextUntil(".level-01").css({ visibility: "visible" });
        $(this)
          .find(".expand-level-01")
          .html("<i class='fa fa-caret-down'></i>");
      } else {
        $(this).nextUntil(".level-01").css({ visibility: "collapse" });
        $(this)
          .find(".expand-level-01")
